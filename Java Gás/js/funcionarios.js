...
function RenderProd() {
    ...
    for(var func in todos) {
        ...
        img.setAttribute('src', 'img/'+p.func+'.png');
        img.setAttribute('alt', `${p.func}`);
        div3.appendChild(img);

        //vai dentro da div4
        var span1 = document.createElement('span');
        span1.setAttribute('id', 'nome');
        span1.innerHTML = `Nome: ${p.Nome}`;
        
        var span3 = document.createElement('span');
        span3.setAttribute('id', 'sal');
        span3.innerHTML = `Salário: ${p.Sal}`;
        var span4 = document.createElement('span');
        span4.setAttribute('id', 'cpf');
        span4.innerHTML = `CPF: ${p.Cpf}`;
...
function edit(indice){
    ...
    var arrEd = JSON.parse(localStorage.getItem('tblFuncionarios')) || [];
